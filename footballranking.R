...
while (i <= length(year)) {
  call <- paste0("https://247sports.com/college/arizona/season/",year[i],"-football/commits/")
  page <- read_html(call)
...
## add size dots for multiple recuits rated the same in same year
dot_sizes <- all_recruits %>%
  group_by(Year, Ranking) %>%
  summarize(count = n(), .groups = "drop")  # 'count' is how many dots at each (Year, Ranking)
...
ggplot(all_recruits, aes(x = Year, y = Ranking)) +
  ...
  # Recruits with size mapped to frequency
  geom_point(aes(size = count), alpha = 0.4, color = "blue") +
  
  # LOESS smoothed trend line
  geom_smooth(method = "loess", se = TRUE, color = "firebrick",
              fill = "pink", linetype = "dashed", alpha = 0.2) +
  
  # Yearly average line + points
  geom_line(data = avg_rank, aes(x = Year, y = Average_Ranking), color = "orange", size = 1.2) +
  geom_point(data = avg_rank, aes(x = Year, y = Average_Ranking), color = "orange", size = 1.8) +
  
  # Top recruit label
  geom_text(data = top_recruits, aes(x = Year, y = Ranking, label = label),
            color = "red", fontface = "bold", size = 1.4, vjust = -2) +
  
  # Elite tier annotation
  geom_hline(yintercept = 90, linetype = "dotted", color = "gray") +
  annotate("text", x = min(all_recruits$Year), y = 91.5,
           label = "Elite Tier (90+)", hjust = 0, color = "gray", size = 2) +
  
  # Scale sizes
  scale_size_continuous(range = c(2, 6)) +  # adjust dot size range
  scale_y_continuous(limits = c(65, 100)) +
  ...
  labs(
    ...
    y = "247Sports Player Ranking",
    size = "Recruits\nat Rating",
    subtitle = "Dot size reflects how many recruits share that score",
    caption = "*excluding transfers"
  ) +
  theme_minimal()
...
plot <- ggplot(all_recruits, aes(x = Year, y = Ranking)) +
  geom_point(aes(text = hover_label), alpha = 0.6, color = "steelblue") +
  geom_smooth(method = "loess", se = TRUE, color = "firebrick",
              fill = "pink", linetype = "dashed", alpha = 0.2) +
  geom_line(data = avg_rank, aes(x = Year, y = Average_Ranking), 
            color = "orange", size = 1.2) +
  geom_point(data = avg_rank, aes(x = Year, y = Average_Ranking), 
             color = "orange", size = 2.2) +
  geom_text(data = top_recruits, aes(x = Year, y = Ranking, label = label),
            color = "darkgreen", fontface = "bold", size = 3, vjust = -1) +
  geom_hline(yintercept = 90, linetype = "dotted", color = "darkgreen") +
  annotate("text", x = min(all_recruits$Year), y = 91.5,
           label = "Elite Tier (90+)", hjust = 0, color = "darkgreen", size = 3) +
  scale_y_continuous(limits = c(65, 100)) +
  labs(
    title = "UA Recruit Rankings: Depth, Trends, and Top Talent by Year",
    subtitle = "Hover over each dot to view player name and rating",
    x = "Recruiting Year",
    y = "Player Ranking",
    caption = "Blue dots = individual recruits • Orange = yearly average • Green labels = top recruit"
  ) +
  theme_minimal()
